...
let
  ...
  syncScript = pkgs.writeScript "kimchi-claude-sync.py" ''
    ...
    store_path = os.path.join(home, ".config", "kimchi", "harness", "models-store.json")
    try:
    ...
    store["anthropic"] = {"type": "oauth", "access": access, "refresh": refresh, "expires": expires}
    ...
    settings_path = os.path.join(home, ".config", "kimchi", "harness", "settings.json")
    try:
